...
def ResidualCategAnnotatedAsAdditional(unique_parent_records_residual_datste, residual_categories_dataset):
    ...
    not_includ_categor_temp = list(zip(sum_num_val_not_incl_categ, unique_parent_records_residual_datste))     
...
## ----------------------------------------------------------------------------------------------------
    ...
    sorted_indices = sorted(list(range(len(second_col))), key=lambda k: second_col[k])        
    ...
    ind = sorted(list(range(len(visual_info[0]))), key = lambda k: [sum(one_category) for one_category in visual_info[0]][k])
    inner = [visual_info[1][x] for x in ind]
    outer_lab = [visual_info[2][x] for x in ind]
    outer_val =  [visual_info[3][x] for x in ind]
    #return(inner, "****", outer_lab, "xxxxxx", outer_val )
    #return(ind, "xxxx", inner, "oooo", outer_lab, "zzzz", outer_val)
...
## ----------------------------------------------------------------------------------------------------
    ...
    #return(outer_ring_colors_limited_gradient,  "outer_ring_colors_limited_gradient")
    ...
    outer_concat_color = np.array(full_list).reshape(int(len(full_list)/3), 3) 
...
#    return(legend_labeling, pie_chart_percent)
    ...
    plt.legend(p_inner[ ::-1], 
               ["%s, %1.1f%%" % (l, s) for l, s in list(zip(np.array(legend_labeling), pie_chart_percent ))[ ::-1]], 
               ncol = 1,
               loc = "upper left", 
